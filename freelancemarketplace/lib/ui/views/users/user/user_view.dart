...
import '../../sellers/seller/content7.dart';
import '../../sellers/seller/header_content1.dart';
import 'about.dart';
...
class UserView extends StackedView<UserViewModel> {
                    ...
                    return Wrap(
                      children: snapshot.data.map<Widget>((item) {
                        return item['status'] == "false"
                            ? SizedBox.shrink()
                            : viewModel.search.text.isEmpty
                                ? listdata(context, item, viewModel)
  ...
  Widget listdata(BuildContext context, item, UserViewModel viewModel) {
    return InkWell(
      onTap: () => viewModel.details(item),
      child: Container(
        padding: const EdgeInsets.all(10),
        margin: const EdgeInsets.all(10),
        decoration: BoxDecoration(
  ...
  Widget searches(String data, UserViewModel viewMode, BuildContext context) {
    return InkWell(
      onTap: () {
  ...
  Future<void> addcomplaint(
      BuildContext context, UserViewModel viewModel) async {
    await showDialog(
      context: context,
  ...
  Future<void> updateuser(BuildContext context, UserViewModel viewModel) async {
    await showDialog(
      context: context,
      builder: (BuildContext context) {
        return Dialog(
